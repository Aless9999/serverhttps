...
public class UserService {
    ...
    public Optional<User> register(String username, String rawPassword) {
        ...
        User user = new User(username,rawPassword,apiKey);
    ...
    public ResponseEntity<RegisterResponse> responseAfterRegister(@Valid RegisterRequest request) {
        Optional<User> createUser = register(request.username(),request.password());
        if(createUser.isPresent()){
            User user = createUser.get();
            RegisterResponse registerResponse = new RegisterResponse("access",
                    user.apikey(),
                    "Hello, "+ user.username()+" ! You is register" );
